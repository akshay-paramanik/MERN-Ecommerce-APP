...
const UserAPI = (token) => {
    ...
    try {
        const res = await axios.patch('http://localhost:5000/user/addcart',
            { product },
            { headers: { Authorization: token } }
        );
...
const findUser = async ()=>{
    try{
        const res = await axios.get('http://localhost:5000/user/viewusers',{
            headers:{Authorization: token}
        })
        setUsers(res.data);
...
const removeFromCart = async (id)=>{
    ...
    // sync with backend
    await axios.patch('http://localhost:5000/user/remove_cart',{cart:updatedCart},{
        headers:{Authorization:token}
    })
